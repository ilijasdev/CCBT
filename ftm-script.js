const APIKEYFTM = 'ckey_999c6c5421f24cd08f32bf5bfb0';

async function getTokenDataFtm() {
    // Covalent API setup
    ...
    const chainIdFTMMainnet = '250';

    const url = new URL(`https://api.covalenthq.com/v1/${chainIdFTMMainnet}/address/${address}/balances_v2/?&key=${APIKEYFTM}`);

    // Get key HTML elements and reset table content
    const ul = document.getElementById('metadata');

    const tableRef = document.getElementById('tokenTableFtm');
    tableRef.innerHTML =
