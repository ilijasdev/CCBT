const APIKEYBSC = 'ckey_999c6c5421f24cd08f32bf5bfb0';

async function getTokenDataBsc() {
    // Covalent API setup
    ...
    const chainIdBSCMainnet = '56';

    const url = new URL(`https://api.covalenthq.com/v1/${chainIdBSCMainnet}/address/${address}/balances_v2/?&key=${APIKEYBSC}`);

    // Get key HTML elements and reset table content
    const ul = document.getElementById('metadata');

    const tableRef = document.getElementById('tokenTableBsc');
    tableRef.innerHTML =
